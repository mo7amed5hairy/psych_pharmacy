        ...
        <div id="medicineRowsContainer">
            ...
            <div class="grid grid-cols-1 md:grid-cols-7 gap-3 medicine-row items-end">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="medicines[0][name]" id="medicineName" class="input" placeholder="مثال: باراسيتامول 500" required>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="number" name="medicines[0][price_hotline]" id="priceHotline" class="input" step="0.01" min="0" placeholder="1.25" required>
                </div>
                <div>
                    <label class="label">سعر الارتباط</label>
                    <input type="number" name="medicines[0][price_contract]" id="priceContract" class="input" step="0.01" min="0" placeholder="0.95" required>
                </div>
                <div>
                    <label class="label">سعر العيادة</label>
                    <input type="number" name="medicines[0][price_clinic]" id="priceClinic" class="input" step="0.01" min="0" placeholder="1.10" required>
                </div>
                <!-- Add Button for first row -->
                <div class="flex items-center justify-center" id="addBtnContainer">
                    <button type="button" onclick="addMedicineRow()" class="btn btn-primary w-full h-10 flex items-center justify-center" title="إضافة صنف آخر">
...
<div class="card p-5">
    <div class="flex items-center justify-between mb-3 flex-wrap gap-2">
        ...
        <div class="relative">
            <input type="text" id="searchInput" class="input max-w-xs" placeholder="🔎 بحث..." onkeyup="searchMedicines(this.value)">
        </div>
    </div>
    <div class="table-wrap">
        <table class="data" id="medicinesTable">
            <thead>
                <tr>
                    <th>#</th>
                    <th>الصنف</th>
                ...
                <tr data-id="{{ $medicine->id }}">
                    ...
                    <td>
                        <div class="flex gap-2">
                            <button type="button" onclick="editMedicine({{ $medicine->id }}, '{{ $medicine->name }}', {{ $medicine->unit_type_id }}, {{ $medicine->price_hotline }}, {{ $medicine->price_contract }}, {{ $medicine->price_clinic }})" class="btn btn-ghost">
                                <i class="fas fa-edit"></i>
                            </button>
                            <form method="POST" action="{{ route('medicines.destroy.post', $medicine) }}" class="inline delete-form">
                                @csrf

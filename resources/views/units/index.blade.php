...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
    ...
    <div class="card p-5 lg:col-span-2">
        <h3 class="font-extrabold text-slate-900 mb-3">الوحدات المسجلة</h3>
        <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
            @foreach($unitTypes as $unit)
            <div class="border border-slate-200 rounded-xl p-3 flex items-center justify-between">
                <span class="font-bold text-slate-800">{{ $unit->name }}</span>
                <div class="flex gap-1">
                    <button onclick="editUnit({{ $unit->id }}, '{{ $unit->name }}')" class="btn btn-ghost px-2 py-1 text-xs">
                        <i class="fas fa-edit"></i>
                    </button>
                    <form method="POST" action="{{ route('units.destroy.post', $unit) }}" class="inline" onsubmit="event.preventDefault(); if(confirm('هل أنت متأكد من حذف هذه الوحدة؟')) { showLoader(); this.submit(); }">
                        @csrf
